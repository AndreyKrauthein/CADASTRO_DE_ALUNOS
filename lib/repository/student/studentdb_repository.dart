...
class StudentDBRepository implements StudentRepository {
  ...
  StudentDBRepository() {
    dbLocal = DBLocal(table: "students");
  }
  ...
  Future<int> update(
      ...
      required String condititions,
      required List conditionValues}) async {
    Database database = await dbLocal.getConnection();
    int result = await database.update(dbLocal.table, entity.toMap(),
        where: condititions, whereArgs: conditionValues);
    database.close();
    return result;
  }
}
